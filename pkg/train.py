...
def train_text2mel(load_trained):
    ...
    for loop_cnt in range(int(Hyper.num_batches / batch_maker.num_batches() + 0.5)):
        ...
        for bi in bar:
            ...
            multi_atten = atten_guide * graph.attention * atten_mask

            loss_atten = criterion_atten(multi_atten, torch.zeros_like(graph.attention)) * dynamic_guide
            ...
            if global_step % 100 == 0:
                gs = 0
                #plot_spectrum(spectrum, name, gs, colorbar=False, dir=Hyper.logdir)
